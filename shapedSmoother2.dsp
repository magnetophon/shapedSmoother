...
shapedSmoother(x) =
  ...
  :(_,_,!,_)
with {
  env(prev,prevPhase,prevTotalStep,x) =
    result
  , newPhase
  , totalStep
  , x
  ...
  active = attacking | releasing;
    ...
    totalStep =
      select2(releasing,
        (x-prev) : min(prevTotalStep) * attacking,
        (x-prev) : max(prevTotalStep) * releasing
      );
    ...
    result =
      select2(attacking,
        select2(releasing, x, shaped:min(x)),
        shaped:max(x)
      );
  };
};
